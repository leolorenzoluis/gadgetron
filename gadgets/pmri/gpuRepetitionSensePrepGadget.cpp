#include "gpuRepetitionSensePrepGadget.h"
#include "Gadgetron.h"
...
namespace Gadgetron{
  ...
  gpuRepetitionSensePrepGadget::gpuRepetitionSensePrepGadget()
    : slices_(-1)
    , sets_(-1)
    , device_number_(-1)
  ...
  int gpuRepetitionSensePrepGadget::process_config(ACE_Message_Block* mb)
  {
    ...
    previous_repetition_no_ = boost::shared_array<long>(new long[slices_*sets_]);
  ...
  int gpuRepetitionSensePrepGadget::
          ...
          GadgetContainerMessage< hoNDArray<float> > *m3)                   // traj/dcw
  {
    ...
    if( output_timing_ )
      process_timer = boost::shared_ptr<GPUTimer>( new GPUTimer("gpuRepetitionSensePrepGadget::process()") );
    ...
    if (previous_readout_no_[idx] >= 0) {
      ...
      if ( repetition ==  previous_repetition_no_[idx]) { 
    ...
    previous_repetition_no_[idx] = repetition;
  ...
  gpuRepetitionSensePrepGadget::extract_samples_from_queue ( ACE_Message_Queue<ACE_MT_SYNCH> *queue, 
  ...
  {    
    ...
    for (unsigned int p=0; p<readouts_buffered; p++) {
      ...
      if (queue->dequeue_head(mbq) < 0) {
        ...
        throw std::runtime_error("gpuRepetitionSensePrepGadget::extract_samples_from_queue: dequeing failed");	
      }
      ...
      if (!daq) {
        ...
        throw std::runtime_error("gpuRepetitionSensePrepGadget::extract_samples_from_queue: failed to interpret data");	
      }
  ...
  gpuRepetitionSensePrepGadget::extract_trajectory_from_queue ( ACE_Message_Queue<ACE_MT_SYNCH> *queue, 
                                                             bool sliding_window, unsigned int set, unsigned int slice )
  {    
    if(!queue) {
      GADGET_DEBUG1("Illegal queue pointer, cannot extract trajectory\n");
      throw std::runtime_error("gpuRepetitionSensePrepGadget::extract_trajectory_from_queue: illegal queue pointer");	
    }

    if(queue->message_count()==0) {
      GADGET_DEBUG1("Empty queue, cannot extract trajectory\n");
      throw std::runtime_error("gpuRepetitionSensePrepGadget::extract_trajectory_from_queue: empty queue");	
    }
    ...
    for (unsigned int p=0; p<readouts_buffered; p++) {      
      ...
      if (queue->dequeue_head(mbq) < 0) {
        GADGET_DEBUG1("Message dequeue failed\n");
        throw std::runtime_error("gpuRepetitionSensePrepGadget::extract_trajectory_from_queue: dequeing failed");	
      }
      ...
      if (!daq) {
        GADGET_DEBUG1("Unable to interpret data on message queue\n");
        throw std::runtime_error("gpuRepetitionSensePrepGadget::extract_trajectory_from_queue: failed to interpret data");	
      }
  ...
  void gpuRepetitionSensePrepGadget::extract_trajectory_and_dcw_from_queue
  ...
  gpuRepetitionSensePrepGadget::duplicate_array( GadgetContainerMessage< hoNDArray<T> > *array )
  {
  ...
  void gpuRepetitionSensePrepGadget::reconfigure(unsigned int set, unsigned int slice)
  {    
    unsigned int idx = set*slices_+slice;
  ...
  GADGET_FACTORY_DECLARE(gpuRepetitionSensePrepGadget)
}
